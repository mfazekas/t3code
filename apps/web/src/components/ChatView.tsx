...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const { available: availableProviderOptionsForPicker } = useMemo(
    () => splitProviderOptionsByAvailability(providerStatuses),
    [providerStatuses],
  );
  const lockedProvider: ProviderKind | null = hasThreadStarted
    ? (sessionProvider ?? selectedProviderByThreadId ?? null)
    : null;
  const firstAvailableProvider = availableProviderOptionsForPicker[0]?.value ?? "claudeCode";
  const selectedProvider: ProviderKind =
    lockedProvider ?? selectedProviderByThreadId ?? firstAvailableProvider;
  ...
  const searchableModelOptions = useMemo(
    () =>
      availableProviderOptionsForPicker
    ...
    [lockedProvider, modelOptionsByProvider, availableProviderOptionsForPicker],
  );
  ...
  const onComposerDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    if (nonImageFiles.length > 0) {
      ...
      composerEditorRef.current?.insertText(insertion);
    } else {
      focusComposer();
    }
  };
...
/**
 * Compute available/unavailable provider options dynamically based on
 * server-reported health statuses.  Providers without a health entry
 * (e.g. cursor) are treated as unavailable.
 */
function splitProviderOptionsByAvailability(statuses: ReadonlyArray<ServerProviderStatus>) {
  const available: Array<{ value: ProviderKind; label: string }> = [];
  const unavailable: Array<{ value: ProviderPickerKind; label: string }> = [];
  for (const option of PROVIDER_OPTIONS) {
    const status = statuses.find((s) => s.provider === option.value);
    if (status?.available) {
      available.push(option as { value: ProviderKind; label: string });
    } else {
...
const ProviderModelPicker = memo(function ProviderModelPicker(props: {
  ...
  const { available: availableProviderOptions, unavailable: unavailableProviderOptions } = useMemo(
    () => splitProviderOptionsByAvailability(props.providerStatuses),
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          {props.provider === "codex" && props.serviceTierSetting && shouldShowFastTierIcon(props.model, props.serviceTierSetting) ? (
